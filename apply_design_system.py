...
def apply_fixes():
    ...
    content = content.replace(
        'class="px-2.5 py-1.5 sm:px-4 sm:py-1.5 bg-slate-900 hover:bg-slate-800 border border-slate-900 rounded-xl text-[9px] sm:text-[10px] font-black transition-all text-white shadow-lg shadow-slate-900/10 whitespace-nowrap uppercase tracking-widest"',
    ...
    content = content.replace(
        'text-4xl sm:text-7xl lg:text-8xl font-black mb-6 sm:mb-8 leading-tight break-words tracking-tight',
        'text-4xl sm:text-5xl lg:text-6xl font-extrabold mb-5 sm:mb-6 leading-tight break-words tracking-tight'
    )
    content = content.replace(
        'text-base sm:text-2xl text-slate-600 mb-10 sm:mb-14 max-w-3xl mx-auto px-1 leading-relaxed font-light',
        'text-base sm:text-lg text-slate-500 mb-8 sm:mb-10 max-w-2xl mx-auto px-1 leading-relaxed'
    )
    content = content.replace(
        'class="btn-shine px-12 py-5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold text-lg transition-all transform hover:scale-105 shadow-2xl shadow-slate-900/20 tracking-wide"',
    ...
    # 3. Logo Carousel - Safe replace
    content = re.sub(
        r'<div class="logo-card">\s*<img src="([^"]+)" alt="([^"]+)" class="[^"]+">\s*<span class="[^"]+">([^<]+)</span>\s*</div>',
        r'<div class="flex-shrink-0 opacity-50 hover:opacity-100 transition-opacity duration-300 px-4 flex items-center gap-2"><img src="\1" alt="\2" class="h-6 w-auto grayscale hover:grayscale-0 transition-all duration-300"><span class="font-bold text-slate-500 text-sm">\3</span></div>',
        content
    )
    # Also handle logo cards without text if any
    content = re.sub(
        r'<div class="logo-card">\s*<img src="([^"]+)" alt="([^"]+)" class="[^"]+">\s*</div>',
        r'<div class="flex-shrink-0 opacity-50 hover:opacity-100 transition-opacity duration-300 px-4"><img src="\1" alt="\2" class="h-6 w-auto grayscale hover:grayscale-0 transition-all duration-300"></div>',
        content
    )
    ...
    content = re.sub(
        r'class="feature-card-glow group relative bg-white border border-slate-200/60 rounded-\[2rem\] p-8 sm:p-10 shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:shadow-purple-500/10 transition-all duration-500 hover:-translate-y-2"',
        r'class="group bg-white p-6 hover:bg-slate-50 transition-colors duration-200"',
        content
    )
    # Feature icons bg from rounded-2xl to rounded-xl
