...
import { SenderIDVerificationPage } from './SenderIDVerificationPage.js';
...
export const App = () => {
    ...
    const senderIdVerificationMatch = window.location.hash.match(/^#\/sender-ids\/verify\/(\d+)/);
    const senderIdVerificationId = senderIdVerificationMatch ? Number(senderIdVerificationMatch[1]) : null;
    ...
    const renderPage = () => {
        switch (currentPage) {
            ...
            case 'sender-ids':
                return senderIdVerificationId
                    ? html`<${SenderIDVerificationPage} requestId=${senderIdVerificationId} />`
                    : html`<${SenderIDManagement} />`;
            case 'templates':
